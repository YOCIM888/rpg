...
export function generateWeather() {
  ...
  if (roll < 0.40) gameState.weather = "晴天";
  else if (roll < 0.70) gameState.weather = "阴天";
  else if (roll < 0.80) gameState.weather = "雨天";
  else if (roll < 0.85) gameState.weather = "酷暑";
  else if (roll < 0.90) gameState.weather = "大雾";
  else if (roll < 0.95) gameState.weather = "酸雨";
  else gameState.weather = "沙暴";
}
...
export function applyWeatherEffects(steps) {
  for (let i = 0; i < steps; i++) {
    switch (gameState.weather) {
      case "雨天":
        gameState.crash = Math.min(100, (gameState.crash || 0) + 3);
        break;
      case "酷暑":
        gameState.hydration = Math.max(0, gameState.hydration - 8);
        break;
      case "沙暴":
