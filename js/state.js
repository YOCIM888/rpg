...
import { TIME_PHASES, MELEE_WEAPONS, FOODS, DRINKS, BACKPACK_TYPES, AFFINITY_THRESHOLDS, AFFINITY_MAX, GAME_CONSTANTS, DEFAULT_ITEM_IDS } from './config.js';
...
const INITIAL_STATE = {
  ...
  other: [],
  cargo: [],
  npcAffinity: { v: 0, xiaohan: 0, lili: 0, nurseZombie: 0, leader: 0 },
  npcQuestsDone: { v1: false, v2: false, v3: false, v4: false, v5: false, v6: false, xh0: false, xh1: false, xh2: false, xh3: false, xh4: false, xh5: false, llGift: false },
  lastBegDay: 0,
  lastClimbDay: 0,
  lastPickFruitDay: 0,
  lastCaveDay: 0,
  ...
  warehouseLevel: 0,
  warehouse: [],
  crops: [null, null, null, null, null],
  weather: "晴天",
  castleDebt: null,
  ...
  lastChatDay: 0,
  story: "开场白：丧尸病毒爆发已经一周了，你蜷缩在幸存者帐篷里，听着外面隐约传来的丧尸低吼声。饥饿、口渴和恐惧同时袭来，你必须想办法活下去……貌似有人组建了曙光阵地，也有人创建了末日王国。",
  options: [{ text: "睡觉", action: "sleep" }, { text: "进食", action: "eat" }, { text: "饮水", action: "drink" }, { text: "医疗", action: "medicine" }, { text: "装备", action: "equip" }, { text: "出门", action: "goOut" }, { text: "丢弃", action: "discard" }, { text: "存档", action: "save_game" }, { text: "伙伴收获", action: "partner_harvest" }, { text: "基地建设", action: "base_build" }],
  phase: "choose",
  gameOver: false,
  combatState: null,
};
...
export function advanceTime(steps = 1) {
  ...
  while (gameState.crashTurns >= GAME_CONSTANTS.SURVIVAL.CRASH_THRESHOLD_TURNS) {
    ...
    if (gameState.crash < 100) {
      gameState.crash = Math.min(gameState.crash + GAME_CONSTANTS.SURVIVAL.CRASH_PER_CYCLE, 100);
...
export function updateStatusEffects() {
  ...
  if (gameState.hunger <= 0) {
    ...
    gameState.status = "口渴";
  } else if (gameState.infection > GAME_CONSTANTS.SURVIVAL.INFECTION_THRESHOLD) {
    gameState.status = "感染";
    messages.push("你感到身体在被病毒侵蚀……");
  } else if (gameState.crash > GAME_CONSTANTS.SURVIVAL.CRASH_STATUS_THRESHOLD) {
    gameState.status = "崩溃";
    messages.push("你的精神濒临崩溃……");
  } else if (gameState.health < GAME_CONSTANTS.SURVIVAL.SEVERE_INJURY_THRESHOLD) {
    gameState.status = "重伤";
...
export function applyWeatherEffects(steps) {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (effects.health) gameState.health = Math.max(0, gameState.health + effects.health);
  }
}
