      ...
      if (state._blindTurns > 0 && Math.random() < 0.5) {
        ...
        if (Math.random() < zombie.dodge) {
          ...
          do {
            ...
            if (comboHits === 0) {
              combatLog.push({ round, text: `你用${state.meleeWeapon.name}攻击，造成${state.meleeWeapon.damage}点伤害。（HP:${Math.max(0, zombie.currentHp)}）` });
            } else {
              combatLog.push({ round, text: `连击！追加攻击！造成${state.meleeWeapon.damage}点伤害！（${comboHits}连击）（HP:${Math.max(0, zombie.currentHp)}）` });
            }
...
function handleBanditEncounter() {
  ...
  options.push({ text: "逃跑（25%）", action: "combat_npc_flee" });
  setOptions(options);
  return;
}
...
function handleNpcCombat(npc, combatMode = "melee") {
  ...
  if (npc.hp <= 0) {
    ...
    if (npc.name === "马三") {
      ...
      s.other.push({ ...SPECIAL_ITEMS.silence_badge });
      ...
      const bankState = getState();
      bankState.castleDebt = null;
      bankState.castleDebtTriggered = false;
...
function handleWanderingTraderAction(input) {
  ...
  if (action === "trader_weapon") {
    ...
    const weaponOptions = TRADER_WEAPON_SHOP.map((item, i) => {
      ...
      return { text: `${weaponName}（${typeLabel} ${item.costMin}~${item.costMax}支香烟）`, action: `buy_weapon_${i}`, index: i };
    });
  ...
  if (action === "trader_seed") {
    ...
    const added = addItem({ ...BUILDING_MATERIALS.find(m => m.id === DEFAULT_ITEM_IDS.seed) });
    if (added) {
...
function handleDoctorAction(input) {
  ...
  if (action === "doctor_psychology") {
    ...
    if (foodItems1.length < GAME_CONSTANTS.DOCTOR.PSYCHOLOGY_FOOD_COST) {
      setStory(`你的食物不够，心理治疗需要${GAME_CONSTANTS.DOCTOR.PSYCHOLOGY_FOOD_COST}份食物。`);
      handleDoctorEncounter();
      return;
    }
    for (let i = 0; i < GAME_CONSTANTS.DOCTOR.PSYCHOLOGY_FOOD_COST; i++) {
      state.food.pop();
    }
  ...
  if (action === "doctor_fruit") {
    ...
    if (fruits.length < GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST) {
      setStory(`医生摇摇头：\"你只有${fruits.length}份水果，我需要${GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST}份。去多找些水果再来。\"`);
      advanceTime(1);
      updateStatusEffects();
      checkDeath();
      showExploreOptionsState();
    ...
    let removed = 0;
    for (let i = state.other.length - 1; i >= 0 && removed < GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST; i--) {
    ...
    if (info.arr.length < GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST) {
      setStory(`医生摇摇头：\"你只有${info.arr.length}份${info.label}，我需要${GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST}份。\"`);
      advanceTime(1);
      updateStatusEffects();
      checkDeath();
      showExploreOptionsState();
    ...
    for (let i = 0; i < GAME_CONSTANTS.DOCTOR.TREATMENT_ITEM_COST; i++) {
      info.arr.pop();
    }
  }
