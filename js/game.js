...
function handleExplore() {
  ...
  if (Math.random() < 0.1) {
    const npcType = Math.random() < 0.7 ? 'survivor' : 'bandit';
    if (npcType === 'survivor') {
...
function handleTradeChoice(input) {
  ...
  if (inventory.length === 0) {
    setStory(`你没有${label}可以进行交易。交易失败。`);
    state._trade = null;
    advanceTime(1);
    updateStatusEffects();
    checkDeath();
  ...
  setStory(`幸存者愿意用子弹换你的${label}（每份换${trade.ammoPerItem}发${trade.ammoType}）。\n你有${inventory.length}份${label}。请输入你想交易的份数（1-${inventory.length}）：`);
  setOptions([{ text: `交易（输入 1-${inventory.length}）`, action: "confirm" }]);
}
...
function handleTradeInput(input) {
  ...
  const inventory = state[type];
  const maxTrade = inventory.length;

  if (isNaN(input) || input < 1) {
    setPhase("trade_input");
    setStory(`幸存者愿意用子弹换你的${label}（每份换${trade.ammoPerItem}发${trade.ammoType}）。\n你有${inventory.length}份${label}。请输入交易份数（1-${maxTrade}）：`);
    setOptions([{ text: `交易（输入 1-${maxTrade}）`, action: "confirm" }]);
    return;
  }

  if (input > maxTrade) {
    setStory(`你没有那么多${label}（你有${maxTrade}份，无法交易${input}份）。交易失败。`);
    state._trade = null;
    state._tradeType = null;
    advanceTime(1);
    updateStatusEffects();
    checkDeath();
    showExploreOptionsState();
  ...
  const count = input;
  const ammoReceived = count * trade.ammoPerItem;

  for (let i = 0; i < count; i++) {
    state[type].pop();
  }

  addItem({ id: trade.ammoType, name: trade.ammoType, type: "ammo", count: ammoReceived });

  setStory(`交易成功！你用${count}份${label}换来了${ammoReceived}发${trade.ammoType}。`);
  state._trade = null;
  state._tradeType = null;
