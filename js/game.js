...
function handleSleep() {
  ...
  advanceTime(4);
  updateStatusEffects();
  setStory(`你沉沉睡去，醒来后感觉精神好了一些。崩溃减轻了${reduction}%。`);
  checkDeath();
  showHomeOptions();
}
...
function handleSelectionPhase(input, type, label) {
  const state = getState();
  const optionIndex = input - 1;
  if (optionIndex < 0 || optionIndex >= state.options.length) {
    return;
...
function handleFoodAction(input) {
  handleSelectionPhase(input, "food", "食物");
}

function handleDrinkAction(input) {
  handleSelectionPhase(input, "drinks", "饮品");
}
