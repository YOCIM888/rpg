/* ============================================================
   ...
   → 幸存者阵地 → 阵地 NPC（V/小涵/莉莉）→ 交易 → 装备 → 丢弃 → 路由分发
   ============================================================ */
...
function showExploreOptionsState() {
  ...
  if (state.currentMap && state.currentMap.id === "幸存者阵地") {
    showOutpostOptions();
    return;
  }
  setPhase("explore");
...
function handleExplore() {
  ...
  if (Math.random() < map.encounterRate) {
    ...
    options.push({ text: "逃跑", action: "combat_flee" });
    setOptions(options);
    return;
  } else {
...
function handleCombat(zombieDef, combatMode = "melee") {
  ...
  if (currentMode === "ranged" && state.rangedWeapon) {
    const ammoId = state.rangedWeapon.ammoType;
    const ammo = state.ammo.find(a => a.id === ammoId);
  ...
  while (zombie.currentHp > 0 && state.health > 0) {
    ...
    if (currentMode === "ranged" && Math.random() < 0.5) {
      playerDodged = true;
      combatLog.push({ round, text: "你保持距离，闪避了丧尸的攻击！" });
    }
...
function handleBanditEncounter() {
  ...
  options.push({ text: "逃跑", action: "combat_npc_flee" });
  setOptions(options);
}
...
function handlePreCombatNpcChoice(input) {
  ...
  if (action === "combat_npc_flee") {
    if (Math.random() < 0.10) {
      ...
      const died = handleNpcCombat(npc, "melee");
      if (died) return;
      advanceTime(1);
      updateStatusEffects();
      checkDeath();
  ...
  if (action === "combat_npc_ranged") {
    ...
    setOptions([{ text: "战斗进行中...", action: "none" }]);
    const npc = state._pendingNpc;
    const died = handleNpcCombat(npc, "ranged");
    if (died) return;
    advanceTime(1);
    updateStatusEffects();
    checkDeath();
...
function handleNpcCombat(npc, combatMode = "melee") {
  ...
  while (npc.hp > 0 && state.health > 0) {
    ...
    if (currentMode === "ranged" && Math.random() < 0.5) {
      playerDodged = true;
      combatLog.push({ round, text: "你保持距离，闪避了攻击！" });
    }
...
function getAvailableQuest(npcId) {
  if (npcId === "v") {
    ...
    if (isQuestDone("v3") && !isQuestDone("v4") && aff >= 150) return { id: "v4", ...getNpcConfig("v").quests.v4 };
    return null;
  }
  if (npcId === "xiaohan") {
    const aff = getNpcAffinity("xiaohan");
    if (!isQuestDone("xh1") && aff >= 50) return { id: "xh1", ...getNpcConfig("xiaohan").quests.xh1 };
    if (isQuestDone("xh1") && !isQuestDone("xh2") && aff >= 100) return { id: "xh2", ...getNpcConfig("xiaohan").quests.xh2 };
    return null;
  }
  if (npcId === "lili") {
...
function handleEquipSubAction(input) {
  ...
  if (action === "ranged") {
    ...
    const options = rangedItems.map((item, i) => ({
      text: `${item.name}（伤害:${item.damage} 耐久:${item.integrity} 弹药:${item.ammoType}）`,
