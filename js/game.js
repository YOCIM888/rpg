...
function showExploreOptionsState() {
  ...
  if (state.currentMap) {
    if (state.currentMap.id === "山顶废弃瞭望塔") {
      ...
      opts.push({ text: "蒙面人（不友好）", action: "masked_man" });
    } else if (state.currentMap.id === "废弃工厂仓库") {
      opts.push({ text: "仓库守护者", action: "warehouse_guard_interact" });
    } else if (state.currentMap.id === "城郊大型医院") {
      opts.push({ text: "带有理智的护士丧尸", action: "nurse_zombie_interact" });
    } else if (state.currentMap.id === "废弃警察局") {
      opts.push({ text: "翻找证物室", action: "police_raid" });
    } else if (state.currentMap.id === "军事检查站") {
      opts.push({ text: "神经质老兵", action: "veteran_interact" });
...
function handleSleep() {
  ...
  const healthRecovery = Math.floor(Math.random() * 21) + 60;
    ...
    if (!isCrippled) {
      if (Math.random() < zombie.dodge) {
        ...
        if (state.meleeWeapon.currentDurability <= 0 && state.meleeWeapon.id !== "拳头") {
          breakMsg = `${state.meleeWeapon.name}耐久耗尽，彻底损坏了！`;
  ...
  if (npc.hp <= 0) {
    ...
    if (npc.name === "亡命之徒") {
      ...
      if (addedAk && addedAmmo) {
        combatLog.push({ round: "胜利", text: "亡命之徒身上掉落：AK47×1、7.62×39mm×30" });
      } else {
        combatLog.push({ round: "胜利", text: "亡命之徒身上掉落了一些装备，但你的背包空间不足，部分物品无法携带。" });
      }
    ...
    if (npc.type === "wanderingTrader") {
      ...
      if (droppedCigs.length > 0) {
        combatLog.push({ round: "胜利", text: `流浪商人身上掉落香烟：${droppedCigs.join("、")}（${droppedCigs.length}支）` });
      }
...
function handleWanderingTraderEncounter() {
  setStory("一个背着大包小包的流浪商人出现在你面前。他咧嘴一笑：\"嘿，想不想做点买卖？我这儿什么都有！\"");
  ...
  setOptions([
    ...
    { text: "背刺（偷袭流浪商人）", action: "trader_backstab" },
...
function handleWanderingTraderAction(input) {
  ...
  if (action === "trader_backstab") {
    ...
    const rangedName = state.rangedWeapon ? state.rangedWeapon.name : "无";
    const ammoInfo = getRangedAmmoInfo(state);
    setStory(`你决定铤而走险，偷袭这个流浪商人！但他显然早有防备……\n\n【近战】${meleeName}\n【远程】${rangedName} ${ammoInfo}`);
    const options = [
      { text: "近战作战", action: "combat_npc_melee" }
    ];
    if (canRangedCombat(state)) {
  ...
  if (action === "trader_ammo") {
    if (state.cargo.length === 0) {
      setStory("流浪商人瞥了你一眼：\"你没香烟还想换子弹？去去去！\"");
      showExploreOptionsState();
      return;
    }
    setPhase("trader_buy_ammo");
    setStory(`流浪商人掏出一个弹药箱：\"说吧，要什么子弹？每支香烟换3~6发。\"\n你有${state.cargo.length}支香烟。`);
  ...
  if (action === "trader_weapon") {
    if (state.cargo.length === 0) {
      setStory("流浪商人摇摇头：\"一支香烟都没有，我可不会白送武器。\"");
      showExploreOptionsState();
      return;
    }
    setPhase("trader_buy_weapon");
    setStory(`流浪商人拍了拍他的货箱：\"武器可不便宜，远程更贵！\"\n你有${state.cargo.length}支香烟。`);
    setOptions([
      { text: "G17 手枪（远程 3~5支香烟）", action: "buy_g17", index: 0 },
      { text: "GP100 手枪（远程 3~5支香烟）", action: "buy_rugergp100", index: 1 },
      { text: "消防斧（近战 2~3支香烟）", action: "buy_fireaxe", index: 2 },
      { text: "棒球棍（近战 2~3支香烟）", action: "buy_baseballbat", index: 3 },
  ...
  if (action === "trader_food" || action === "trader_drink" || action === "trader_fruit") {
    ...
    const added = addItem(lootItem);
    if (added) {
      setStory(`流浪商人用${lootItem.name}换走了你一支香烟。`);
    } else {
      state.cargo.push(cig);
      setStory(`背包已满，交易失败。流浪商人把香烟还给了你。`);
    }
    advanceTime(1);
    updateStatusEffects();
    checkDeath();
...
function handleTraderBuyWeapon(input) {
  ...
  switch (option.action) {
    case "buy_g17":
      ...
      costMin = 3; costMax = 5; costLabel = "远程";
      break;
    case "buy_rugergp100":
  ...
  if (state.cargo.length < cost) {
    setStory(`流浪商人摇摇头：\"这${costLabel}武器要${cost}支香烟，你只有${state.cargo.length}支，不够啊。\"`);
    showExploreOptionsState();
    return;
  }
  ...
  if (!added) {
    ...
    setStory(`交易成功！你用${cost}支香烟买下了一把${weaponDef.name}。流浪商人吹了个口哨。`);
  }
...
function handleOutlawInteract() {
  ...
  if (state.outlawKilled) {
    setStory("亡命之徒已经被你亲手杀死了，这里只留下血迹和冰冷的尸体，你不敢看他，虽然是末世，但你还是做了这样的事情。");
    showExploreOptionsState();
    return;
  }
  setPhase("explore");
  setStory("一个满身纹身的壮汉靠在墙边，眼神凶狠地盯着你。他手里握着一把枪，看起来非常不好惹。");
  showOutlawMenu();
}
...
function handleOutlawFight() {
  ...
  setPhase("pre_combat_npc");
  setStory("亡命之徒冷笑一声，拔出了武器。\"来啊，让我看看你有什么本事！\"");
  setOptions([
    { text: "近战作战", action: "combat_npc_melee" },
...
function handleMechanicInteract() {
  setPhase("explore");
  setStory("一个满身油污的中年男人正在摆弄一堆零件，看到你后放下了手中的扳手。");
  showMechanicMenu();
}
...
function handleMechanicTrade() {
  const state = getState();
  let wrenchIdx = state.other.findIndex(i => i.id === "大扳手");
  let pipeIdx = -1;
  if (wrenchIdx === -1) {
    pipeIdx = state.other.findIndex(i => i.id === "铁管");
  }
  if (wrenchIdx === -1 && pipeIdx === -1) {
    setStory("你没有什么可以拿来交易的东西。");
    showMechanicMenu();
    return;
  }
  const toolIdx = wrenchIdx !== -1 ? wrenchIdx : pipeIdx;
  const toolName = state.other[toolIdx].name;
  removeItem("other", toolIdx);
  const ammo = AMMO[Math.floor(Math.random() * AMMO.length)];
  ...
  const food = FOODS[Math.floor(Math.random() * FOODS.length)];
  addItem({ ...food });
  setStory(`你用${toolName}与维修工交换了5颗${ammo.name}、${drink.name}和${food.name}。`);
  advanceTime(1);
  updateStatusEffects();
  checkDeath();
...
function handleMaskedMan() {
  const state = getState();
  state.health -= 59;
  setStory("蒙面人恶狠狠地瞪了你一眼，并鸣枪示警，你中弹了，没伤到要害。");
  advanceTime(1);
  updateStatusEffects();
  checkDeath();
  if (!state.gameOver) {
...
function handleWarehouseGuardInteract() {
  setPhase("explore");
  setStory("一个穿着工装的大汉挡在工厂仓库门口，手里拿着一根铁棍。\"此路是我开，此树是我栽……不对，这仓库归我管，谁也不准进！\"");
  setOptions([
...
function handleNurseZombieInteract() {
  ...
  if (state.nurseZombieRescued) {
    setStory("这里已经什么都没有了，护士丧尸已经跟你回家了。");
    showExploreOptionsState();
    return;
  }
  setPhase("explore");
...
function handleNurseZombieFeedConfirm(input) {
  ...
  if (isCanned) {
    ...
    setStory(`她看起来很喜欢吃罐头！好感度 +${gain}。\n\n[好感度：${state.npcAffinity.nurseZombie}/150]`);
  } else {
    setStory("她好像不喜欢吃这个……也许应该试试罐头食品？\n\n" + `[好感度：${state.npcAffinity.nurseZombie || 0}/150]`);
  }
  handleNurseZombieInteract();
}
...
function handleNurseZombieBringHome() {
  ...
  setStory("护士丧尸跟着你回到了幸存者帐篷，她现在是你忠实的伙伴了。");
  advanceTime(1);
  updateStatusEffects();
  showHomeOptions();
}
...
function handleVeteranInteract() {
  setPhase("explore");
  setStory("一个穿着破烂军装的老兵蹲在检查站的角落里，手里紧握着一把步枪，眼神涣散。他看起来精神有些不太正常。");
  setOptions([
...
function handlePartnerHarvest() {
  ...
  if (nurseReady) {
    ...
    storyLines.push(`护士丧尸今天给你带来了${med.name}。`);
  }
...
function handleNpcInteract(npcId) {
  ...
  if (npcId === "lili") {
    opts.push({ text: "回收武器（永久）", action: "npc_recycle" });
...
function getAvailableQuest(npcId) {
  if (npcId === "v") {
    ...
    if (isQuestDone("v2") && !isQuestDone("v3") && aff >= 80) return { id: "v3", ...getNpcConfig("v").quests.v3 };
    if (isQuestDone("v3") && !isQuestDone("v5") && aff >= 120) return { id: "v5", ...getNpcConfig("v").quests.v5 };
    if (isQuestDone("v5") && !isQuestDone("v4") && aff >= 150) return { id: "v4", ...getNpcConfig("v").quests.v4 };
...
function handleNpcRecycle(npcId) {
  ...
  if (meleeItems.length === 0) {
    setStory(`${getNpcConfig(npcId).name}耸耸肩："你背包里没有多余的近战武器可以回收哦。"`);
    handleNpcInteract(npcId);
    return;
  }
  setPhase("npc_gift");
...
function handleNpcRepair(npcId) {
  ...
  if (rangedItems.length === 0) {
    setStory(`${getNpcConfig(npcId).name}歪着头："你都没有远程武器，修什么呀？"`);
    handleNpcInteract(npcId);
    return;
  }

  setPhase("repair_select");
  const opts = rangedItems.map((item, i) => ({ text: item.label, action: "repair_confirm", index: i, repairItem: item }));
